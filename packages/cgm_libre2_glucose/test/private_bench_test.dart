...
void main() {
  // tool/analyze_private_bench.dart refuses to run at all off macOS (see its
  // `Platform.isMacOS` guard: it depends on Darwin's stat64 field layout via
  // raw FFI). Off macOS every call here returns `unsupportedPlatform`, which
  // makes most of these assertions fail outright and — worse — makes a few
  // of them (the ones that already expect a nonzero/66 exit, for unrelated
  // reasons like a bad permission mode or a symlink) pass for the wrong
  // reason instead of actually exercising that behavior. Skip the whole
  // group off macOS rather than let either happen.
  final String? macosOnlySkip = Platform.isMacOS
      ? null
      : 'analyze_private_bench.dart is macOS-only by design (Darwin stat64 '
            'FFI); these assertions cannot run for real on this platform.';

  group('analyze_private_bench', () {
    late Directory temp;
    late File fram, ble;
    setUp(() {
    ...
    test('multiple matching devices and mixed subscription fragments are '
        'rejected', () async {
    ...
    test(
      'out-of-order fragments and nonmonotonic trace are rejected',
      () async {
    ...
    test(
      'escaped duplicate calibration keys cannot replace a binding',
      () async {
        write();
        final encoded = fram.readAsStringSync();
        fram.writeAsStringSync(
          '{"\\u0073chemaVersion":2,${encoded.substring(1)}',
        );
        final result = await run();
        expect(result.$1, 65);
        closed(result);
