...
Foreground glucose alert episodes are stored locally in the restricted health
state store, capped at the most recent 200 transitions. Native builds keep the
store in the backup-excluded application-support directory; the web demo uses
origin-scoped preferences and is not a supported private health-data surface.
The evaluator records only alert type, timestamps, and the associated glucose
value; it does not send notifications, create lock-screen payloads, or upload
data. A future complete delete-all flow must clear this history alongside the
