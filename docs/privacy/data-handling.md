...
The current app does not provide a verified user export workflow. The following
are acceptance requirements for a future export feature, not a statement of
present capability:
...
AI is off by default. Before an AI feature ships, it must disclose the provider
and exact data categories sent, use HTTPS and a reviewed endpoint, minimize data,
version prompt/model/policy, validate output against safety policy, and provide
representative medical/dosing adversarial evaluations. Provider retention is a
third-party risk and must not be described as local-only.
