...
Audited end to end so that no surface can quietly keep reading the empty field:
...
| `CbioHistoryArchive` (app's live decode) | `rawPayload` | yes: `derivedMillimolesPerLitre` and the record's scaled value |
| App reading (`CbioGlucoseSession`, consolidation PR #209) | `record.rawPayload` | yes: `rawValue: record.rawPayload`, `valueMgdl: record.rawPayloadScaled`, `isDisplayProvisional: true` |
| `CbioRawGlucoseRecord.rawProcessed` | offset 6 | **no** — exposed as the firmware's processed word, read by tests only |
| `compareCbioDecode` (harness path) | both, side by side | no — reporting only |
