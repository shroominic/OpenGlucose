...
| iOS backup exclusion             | Restricted data         | Verified for current restricted state on physical iOS device | Dedicated file/migration tests plus build 17 app-container attribute inspection on `Shroominic` (2026-08-14) | `@shroominic` | —                                 |
| Lock-screen redaction            | Restricted data         | Implemented; active-session visual verification pending  | Android service, iOS controller, local tests; physical active-session evidence still required | `@shroominic` | Before external release           |
| Sensitive-log controls           | Restricted data         | Dart logs disabled; WinRT native output suppression implemented, Windows verification pending | Startup-order, log-policy, and Windows workflow-contract tests; Windows compile and device recheck required | `@shroominic` | Before external release           |
...
| Windows preview compatibility    | Desktop BLE preview     | Implemented; CI, backup policy, and physical verification pending | Windows runner, local-state selector, preview workflow, deterministic bond tests | `@shroominic` | Before any Windows support claim  |
