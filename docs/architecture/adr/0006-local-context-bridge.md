...
- provably post-warmup readings from the active ready sensor session;
...
closed for unproven post-warmup, invalid, raw/calibration-only, provisional,
future, duplicate, or mixed-source reading inputs.

Durable attachment facts use an additive SQLite table separate from legacy
`health_events` JSON. They link a manual diary row to an opaque candidate
revision ID, calculation version, and bounded time window without storing
...
- Schema-four attachment facts remain readable, but lack the session
  discriminator needed for a safe stable episode key. They are not reused as
  schema-five episode claims.
