...
- Date: 2026-08-24
- Accountable privacy/safety owner: `@shroominic`
- Related: issue #39
...
| Hallucinated statistics or citations | Typed evidence IDs, local canonical numeric/unit persistence, and local rendering | A selected aggregate can still be a weak observation |
...
The checked-in redacted consent-dialog screenshot is generated by the widget
test fixture at `openhealth/test/ai/ai_settings_test.dart`. It uses synthetic
readings and the test key `sk-preview`; the visible dialog contains no glucose
values, user identifiers, or secret. The same test checks the dialog's
semantics label contains the recipient endpoint, the disclosed aggregate
categories, and the raw-reading exclusion.

![Redacted remote-generation consent dialog](ai-remote-generation-disclosure-redacted.png)
...
exact OS-runtime capture in the host, expanded adversarial evaluation, and
