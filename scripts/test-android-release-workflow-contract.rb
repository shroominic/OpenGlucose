...
assert(workflow.include?("--dart-define-from-file=\"$cbio_defines_path\""), "release APK must receive the CBIO define file")
assert(workflow.include?("rm -f -- \"$keystore_path\" \"$cbio_defines_path\""), "CBIO define file must be removed")
