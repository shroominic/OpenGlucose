...
assert(workflow.include?("CBIO_VENDOR_STREAM_KEY_HEX"), "CBIO stream key must be a protected release secret")
assert(workflow.include?("CBIO_VENDOR_AUTH_MATERIAL_HEX"), "CBIO auth material must be a protected release secret")
assert(workflow.include?("CBIO_VENDOR_AUTH_TRIGGER_HEX"), "CBIO auth trigger must be a protected release secret")
assert(workflow.include?("CBIO_DART_DEFINE_FROM_FILE=\"$cbio_defines_path\""), "TestFlight build must receive the CBIO define file")
assert(release_script.include?("--notify_external_testers false"), "pilot must not auto-notify")
assert(release_script.include?("--dart-define-from-file=$CBIO_DART_DEFINE_FROM_FILE"), "TestFlight script must pass the CBIO define file")
assert(release_script.include?("CBIO_DART_DEFINE_FROM_FILE must have mode 600"), "TestFlight script must validate CBIO define permissions")
